...
class CallGraphVisitor(ast.NodeVisitor):
    ...
    def visit_Lambda(self, node):
        ...
        try:
            ...
            #This can happen when a lambda expression is used as default argument in a function definition
            self.logger.warn('Lambda scope problem. Skipping...')
...
#        self.logger.info("New edge added for Use import %s in %s" % (mod_node, from_node))
        ...
        if isinstance(result_node, Node):  # resolved result
            ...
            if isSelf:
                ...
                try:
                    ...
                    try:
                        nodeName = node.func.id
                    except: #Call of the form: myfunction()()
                        nodeName = '^^^return_value^^^'
                to_node = self.get_node(namespace=None, name=nodeName, ast_node=node, flavor=Flavor.UNSPECIFIED)
                if self.add_uses_edge(from_node, to_node):
                    self.logger.info(
                        "New edge added for Use from %s to %s (call creates an instance)" % (from_node, to_node)
                    )
    ...
    def expand_unknowns(self):
        ...
        for n in self.uses_edges:
            for n2 in self.uses_edges[n]:
                if n2.namespace is None:
                    n3 = [expansion for expansion in self.nodes[n2.name] if expansion.namespace is not None]
                    if (
                        len(n3) == 1
                        #Add edges to all corresponding methods in the inheritance hierarchy, they will be culled later
                        or (len(n3) > 1 and not any([self.checkInheritance(ex1, ex2)['noInheritance'] for ex1, ex2 in zip(n3[:-1], n3[1:])]))
                    ):
                        for to_expansion in n3:
                            new_uses_edges.append((n, to_expansion))
                        removed_uses_edges.append((n, n2))
...
#            self.uses_edges[from_node].remove(to_node)
    ...
    def cull_inherited(self):
        ...
        for from_node in self.uses_edges:
            for to_node in self.uses_edges[from_node]:
                inherited = False
                for candidate in self.uses_edges[from_node]:

                    inheritance = self.checkInheritance(n1=to_node, n2=candidate)
                    if inheritance['noInheritance']:
                        continue


                    if (
                        (inheritance['n1IsInherited'] and not inheritance['n1IsOverridden']) #Cull an inherited method from the child class
                        or (inheritance['n2IsInherited'] and inheritance['n2IsOverridden']) #Cull a base method from the base class because the child class overrides it
                    ):
                        removed_uses_edges.append((from_node, to_node))
                        self.logger.info("Removing inherited edge from %s to %s" % (from_node, to_node))
    ...
    def removeNonMethods(self):
        ...
        for from_node, to_set in self.uses_edges.items():
            for to_node in to_set:
                if to_node.flavor  in [Flavor.NAME, Flavor.ATTRIBUTE]:
                    removedElements.append((from_node, to_node))

        for from_node, to_node in removedElements:
            self.uses_edges[from_node].remove(to_node)

    def checkInheritance(self, n1, n2):
        noInheritance = True
        n1IsInherited = False
        n2IsInherited = False
        n1IsOverridden = False
        n2IsOverridden = False

        if (
                n1.name == n2.name
                and n1.namespace is not None
                and n2.namespace is not None
                and n1.namespace != n2.namespace
        ):
            pn1 = self.get_parent_node(n1)
            pn2 = self.get_parent_node(n2)

            try:
                n1IsInherited = pn2 in self.mro[pn1]
                n2IsInherited = pn1 in self.mro[pn2]
            except:
                n1IsInherited = False
                n2IsInherited = False

            assert not (n1IsInherited and n2IsInherited)
            if n1IsInherited or n2IsInherited:
                noInheritance = False
                n1IsOverridden = n1IsInherited and n1 in self.defines_edges.get(pn1, {})
                n2IsOverridden = n2IsInherited and n2 in self.defines_edges.get(pn2, {})


        return {'noInheritance': noInheritance,
                  'n1IsInherited': n1IsInherited,
                  'n2IsInherited': n2IsInherited,
                  'n1IsOverridden': n1IsOverridden,
                  'n2IsOverridden': n2IsOverridden
                }
