...
class ModelRunner:
    ...
    def train(self):
        ...
        test_dataset = self.test_dataset_reader.get_dataset()
        ...
        self.logdir = os.path.join(self.config.SAVE_PATH, 'logs')

        if self.config.MODEL_PATH:
            print('Loading model...')
            checkpoint = tf.train.Checkpoint(step=tf.Variable(1), optimizer=optimizer, model=self.model)
            checkpoint_manager = tf.train.CheckpointManager(checkpoint, self.config.MODEL_PATH, max_to_keep=20)
            ...
            os.mkdirs(os.path.join(self.logdir, 'train'))
            os.mkdirs(os.path.join(self.logdir, 'test'))

        train_summary_writer = tf.summary.create_file_writer(os.path.join(self.logdir, 'train'))
        test_summary_writer = tf.summary.create_file_writer(os.path.join(self.logdir, 'test'))

        epochs_trained = tf.keras.backend.get_value(checkpoint.save_counter)
        best_f1 = 0
        best_epoch = 0
        best_f1_precision = 0
        best_f1_recall = 0
        ...
        for iteration in range(self.config.NUM_EPOCHS):
            ...
            #test loss
            for input_tensors in test_dataset:
                ...
                if batch_num % self.num_batches_to_log == 0:
                    batches_per_epoch = self.num_training_examples/self.config.BATCH_SIZE
                    total_batch = int(epochs_trained * batches_per_epoch + batch_num)
                    with test_summary_writer.as_default():
                        tf.summary.scalar('loss', sum_loss, step=tf.constant(total_batch, dtype=tf.int64))
            ...
            for input_tensors in dataset:
                ...
                if self.config.USE_MOMENTUM:
                    clipped_gradients, global_norm = tf.clip_by_global_norm(gradients, clip_norm=5)
                ...
                if batch_num % self.num_batches_to_log == 0:
                    ...
                    batches_per_epoch = self.num_training_examples/self.config.BATCH_SIZE
                    total_batch = int(epochs_trained * batches_per_epoch + batch_num)
                    loss_history[total_batch] = sum_loss
            ...
            with train_summary_writer.as_default():
                for step, loss in loss_history.items():
                    tf.summary.scalar('loss', loss, step=tf.constant(step, dtype=tf.int64))
            ...
            results, precision, recall, f1, rouge, bleu = self.evaluate('train')
            with train_summary_writer.as_default():
                tf.summary.scalar('f1', tf.constant(f1, dtype=tf.float32), step=tf.constant(epochs_trained, dtype=tf.int64))
                tf.summary.scalar('Bleu', tf.constant(bleu, dtype=tf.float32), step=tf.constant(epochs_trained, dtype=tf.int64))

            # validate model to calculate metrics or stop training
            print('Test evaluation: ')
            results, precision, recall, f1, rouge, bleu = self.evaluate('test')
    ...
    def evaluate(self, mode='test'):
        ...
        with open(log_file_name, 'w') as output_file, open(ref_file_name, 'w') as ref_file, open(
            ...
            if mode.lower() == 'test':
                ...
                dataset = self.train_dataset_reader.get_dataset()
                dataset = dataset.skip(200).take(372)
            else:
            ...
            for input_tensors in dataset:
                ...
                #num_correct_predictions refers to predictions that are (nearly) identical to the original
        ...
        with open(predicted_file_name, 'r', encoding="ISO-8859-1") as prediction_file:
            predictions = prediction_file.read()

        with open(ref_file_name, 'r', encoding="ISO-8859-1") as ref_file:
            refs = ref_file.read()
            refs = refs.split('\n')
            refs = refs[:-1]

        if not predictions.isspace():
            files_rouge = FilesRouge(predicted_file_name, ref_file_name)
            rouge = 0 # files_rouge.get_scores(avg=True, ignore_empty=True)
        else:
            rouge = 'n/a'
        ...
        bleu = sacrebleu.corpus_bleu(predictions_list, [refs])
        bleu = bleu.precisions[0] * bleu.bp
        print('Bleu:', bleu)

        print("Evaluation time: %sh%sm%ss" % ((elapsed // 60 // 60), (elapsed // 60) % 60, elapsed % 60))
        return num_correct_predictions / total_predictions, precision, recall, f1, rouge, bleu
